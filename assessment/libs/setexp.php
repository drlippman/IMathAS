...
// UNARIES gives all 'proper' unary operations...which will be ! here for complement
const UNARIES = array('!');
...
const BINARIES = array("nn","ominus","uu","-");
...
const ALTS = array( "cap"=> "nn", "and" => "nn", "∩"=>"nn", "\\cap" => "nn",
...
const PRECEDENCE = array('!'=>0,"nn"=>1,"uu"=>1,"ominus"=>1,"-"=>2);
// VENN2 gives the coordinates for the 4 regions of the 2-circle Venn diagram, used in venn2diagram
...
function setexpToPostfix($exp){
	...
	// 0 = nn, 1 = ominus, 2 = uu, etc. (the keys of the BINARIES array)
...
function setexpFillRandSkeleton($skeleton,$vars,$ops){
    ...
    if(array_count_values($skeleton)["*"]==3 && count($vars)>2 && count($ops)>1){
        $var = array_rand(array_flip($vars),3);             // Choose three different variables
        
        $op = array_rand(array_flip($ops),2);               // Choose two different operations
        shuffle($var);
        shuffle($op);
        foreach($skeleton as $k=>$v){
            if($v=="*"){                                    // Replace * with variable
    ...
    if(count($L)>1 && count($R)>1 && end($L)==end($R) && BINARIES[end($L)]!="-"){
        $allowedops = array_diff($ops,array(end($L)));
        $O = $allowedops[array_rand($allowedops)];
    }
    elseif(count($L)>1 && count($R)<3 && BINARIES[end($L)]!="-"){
        $allowedops = array_diff($ops,array(end($L)));
        $O = $allowedops[array_rand($allowedops)];
    }
    elseif(count($R)>1 && count($L)<3 && BINARIES[end($R)]!="-"){
...
function venn2diagram($vars,$shade="",$labels="",$size=200){
    ...
    // Else let A = {1,2}, B = {2,3} and U={0,1,2,3} and compute the members of $shade, the members correspond to the regions of VENN2 constant to shade
    $shade = setexpToPostfix($shade);
    $vars = array($A => array(1,2), $B=>array(2,3));
    $regions = setexpPostfixEvaluate($shade,$vars,array(0,1,2,3));
    $DRAW = "";
    foreach($regions as $region){
        $DRAW = $DRAW.VENN2[$region];
    }
