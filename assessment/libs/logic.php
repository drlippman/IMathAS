...
array_push($allowedmacros,"logiclegend","randlogicexp","makelogicpretty","evaluate","evaluateall","truthtable","logicsteps","equivalent","implies","truthtableanswers");
...
// BONEYARD[b] is an array of 'skeleton' strings containing the all possible postfix binary expressions with b operations.  Variable positions are denoted by *.  Operations by #.
const BONEYARD = array(
					array("*"),
					array("**#"),
					array("**#*#","***##"),
					array("**#*#*#","***##*#","**#**##","***#*##","****###"),
					array("**#*#*#*#","**#*#**##","**#**##*#","**#**#*##","**#***###","***##*#*#","***##**##","***#*##*#","***#*#*##","***#**###","****###*#","****##*##","****#*###","****#*###","*****####"));			
// WEIGHT[b] is an array of weights for each skeleton in BONEYARD[b].  This is useful for getting random expressions, so more interesting ones occur more frequently. Higher weights = higher likelihood of being selected.
const WEIGHT = array(array(1),array(1),array(1,1),array(1,1,3,1,1),array(1,3,2,3,3,2,3,2,2,2,2,2,2,1));
// UNARIES gives all 'proper' unary operations...which is just neg.
const UNARIES = array("neg");
// BINARIES array gives 'proper' binary operations. If more are added later this must be of size 10 or less for this library to function properly!
const BINARIES = array("wedge","oplus","vee","implies","iff");
// ALTS gives other words that can be used by the user and directs them to the proper one above
...
const ALTS = array( "^^"=> "wedge", "and" => "wedge", "∧"=>"wedge", "\\wedge" => "wedge",
                    ...
                    "not" =>"neg", "¬" => "neg", "\\neg"=>"neg");
// PRECEDENCE gives the precidence of the operations. Lower is higher precedence.
const PRECEDENCE = array("neg"=>0,"wedge"=>1,"vee"=>1,"oplus"=>1,"implies"=>2,"iff"=>3);

///////////
// NOTES //
///////////

// In the algorithms, binary operations are replaced with numbers, based on their position in the BINARIES array (ALT words are immediately swapped out for a word in BINARIES)
...
function getOperands($exp){
    if(!is_array($exp)){
        echo "Error in getOperands: expects an array of tokens";
        return false;
    }
    if(count($exp) == 1){
        return $exp[0];
...
function postfixLogicSteps($exp){
    if(!is_array($exp)){
        echo "Error in postfixLogicSteps: Input is not an array";
        return false;
    }
    ...
    if(count($operands) == 1){      // Last operation as a negation
        return array_merge(postfixLogicSteps($operands[0]),array($exp));
    }
    $retval = array_merge(postfixLogicSteps($operands[0]),postfixLogicSteps($operands[1]),array($exp));
    return $retval;
}
...
function postfixEvaluate($exp,$vars,$truths){
	if(!is_array($exp) || !is_array($vars) || !is_array($truths)){
        echo "Error in postfixEvaluate: An input is not an array.";
        return false;
    }
	if(count($vars) != count($truths)){
		echo "Error in postfixEvaluate: vars and truths have mismatched size. Aborting.";
		return FALSE;
	}
	foreach($exp as $token){
		if(ctype_alpha($token) && !in_array($token,$vars)){
			echo "Error in evaluate: expression contains variables not listed in vars array";
			return FALSE;
		}
	}
...
function toPostfix($exp){
	...
	// 0 = wedge, 1 = oplus, 2 = or, etc. (the keys of the BINARIES array)
    $exp = str_replace(array_keys(ALTS),array_values(ALTS),$exp);
    $exp = str_replace(array_values(BINARIES),array_keys(BINARIES),$exp);
    ...
    if($balance < 0){
        echo "Error in toPostfix: Parentheses aren't balanced.";
        return FALSE;
    }
    // Ensures no two variables appear without an operation between them
    if(preg_match('/[A-Za-z][^0-9]*[A-Za-z]/u',$exp)){
        echo "Error in toPostfix: Two adjacent variables appear without a binary operation between them.";
        return false;
    }
    // Ensures no two operations appear without a variable between them
    if(preg_match('/[0-9][^A-Za-z]*[0-9]/u',$exp)){
        echo "Error in toPostfix: Two adjacent binary operations appear without a variable between them.";
        return false;
    }
    $exp = str_split($exp);
    // Convert to postfix.
    ...
    foreach($Q as $token){
        switch(TRUE){
            ...
            case (ctype_digit($token) || $token == "#"):                         // BINARY
                ...
                while((end($oStk) != "(") && (PRECEDENCE[BINARIES[end($oStk)]] <= PRECEDENCE[BINARIES[$token]])){ // POP IF TOP OF OSTK HAS GREATER PRIORITY
                    $rStk[] = array_pop($oStk);
                }
                $oStk[] = $token;
                break;
            ...
            case ($token == ")"):                                             // RIGHT PAREN
                ...
                break;
            default:
                echo "Error in toPostfix: Unexpected token '$token' found.";
                return false;
                break;
        }
    }
	...
	foreach($exp as $token){
        switch(TRUE){
           ...
           case ($token == "-"):                               // UNARY
                ...
                break;
            default:
                echo "Error in toInfix: unexpected token '$token' encountered.";
                return false;
                break;
        }
    }
...
function postfixMakePretty($exp){
    $exp = toInfix($exp);
...
function equivalent($exp1,$exp2){
    $exp1 = toPostfix($exp1);
    $exp2 = toPostfix($exp2);
    $vars = array();
    foreach($exp1 as $token){
        if(preg_match('/[A-Za-z]{1}/',$token) && strlen($token) == 1){
            $vars[] = $token;
        }
    }
    ...
    $retval = (postfixEvaluateAll($exp1,$vars) == postfixEvaluateAll($exp2,$vars)) ? 1 : 0;
    return $retval;
}
...
function implies($exp1,$exp2){
    ...
    $vars = array_unique($vars);
    $eval1 = postfixEvaluateAll($exp1,$vars);
    $eval2 = postfixEvaluateAll($exp2,$vars);
...
function makelogicpretty($exp){
    return postfixMakePretty(toPostfix($exp));
}
...
function logicsteps($exp){
    $exp = toPostfix($exp);
    $retArray = postfixLogicSteps($exp);
    foreach($retArray as $k => $v){
        $retArray[$k] = postfixMakePretty($retArray[$k]);
    }
    return $retArray;
}
...
function truthtable($exp, $showresult=FALSE,$showsteps=TRUE,$offset=0){
    ...
    $steps = $vars;
    if(!$showsteps){
        $steps[] = postfixMakePretty($exp);
    }
    else{
        $steps = array_merge($steps,array_map('postfixMakePretty',postfixLogicSteps($exp)));
    }
    // Table header
    $header = "<table><tr>";
    ...
    for($r = 0; $r < pow(2,count($vars)); $r++){
        ...
        foreach($steps as $step){
            if(!in_array($step,$vars)){
                $bin = postfixEvaluate(toPostfix($step),$vars,$truths);
                $letter = (($bin == "1") ? "T" : "F");
                if($showresult){
...
function truthtableanswers($exp,$showsteps=TRUE){
    ...
    for($r = 0; $r < pow(2,count($vars)); $r++){
        ...
        foreach($steps as $step){
            if(!in_array($step,$vars)){
                $retanswers[] = postfixEvaluate(toPostfix($step),$vars,$truths);
            }
        }
    }
    return $retanswers;
...
function evaluate($exp,$vars,$truths){
	$exp = toPostfix($exp);
    ...
    return postfixEvaluate($exp,$vars,$truths);
}
...
// Output: An ARRAY of size 2^|var| bits. The entry in position i is the result of evaluate when the truth values of vars is the binary representation of i. For example, if $vars = [p,q] then the 2rd position of the output would be evaluate exp when p=1 and q=0 (since 2=10 in binary)
function evaluateall($exp,$vars){
	$exp = toPostfix($exp);
    $vars = (is_array($vars) ? $vars : explode(',',$vars));
    foreach($vars as $var){
        if(strlen($var) != 1 || !preg_match('/[A-Za-z]{1}/u',$var)){
            echo "Error in evaluateall: vars array/list must contain only single alphabetical or greek characters";
            return FALSE;
        }
    }    
    return postfixEvaluateAll($exp,$vars);
}
...
function randlogicexp($vars,$ops,$bnum,$unum){
    ...
    foreach($vars as $var){
        if(strlen($var) != 1 || !preg_match('/[A-Za-z]{1}/u',$var)){
            echo "Error in randlogicexp: vars array/list must contain only single alphabetical or greek characters";
            return FALSE;
        }
    }    
	...
	foreach($ops as $entry){
		if(!in_array($entry,BINARIES) && !in_array($entry,array_keys(ALTS))){
			echo "Error in randlogicexp: Every entry in ops must be a valid binary operation (see manual for a list)";
            return false;
		}
	}
    for($i = 0; $i < count($ops); $i++){
        if(in_array($ops[$i],array_keys(ALTS))){
            $ops[$i] = ALTS[$ops[$i]];
        }
        if(in_array($ops[$i],BINARIES)){
            $ops[$i] = array_search($ops[$i],BINARIES);
        }
    }
	// $bnum check
	...
	//1. Select an appropraite skeleton expression from BONEYARD
	...
	$skeletonList = array();
	foreach($bnum as $b){
		for($n = 0; $n < count(BONEYARD[$b]); $n++){
			for($k = 0; $k < WEIGHT[$b][$n]; $k++){
				$skeletonList[] = BONEYARD[$b][$n];
			}
		}
	}
	$skeleton = $skeletonList[array_rand($skeletonList)];
    // 2. INSERT NEGATIONS //
    ...
    $unum = $unum[array_rand($unum)];
    $negPos = array();
    while(count($negPos)<$unum){
        $rand = rand(1,strlen($skeleton));
    ...
    return postfixMakePretty(randomFillSkeleton($skeleton,$vars,$ops));
}
...
// Reason: Used in randlogicexp
function randomFillSkeleton($skeleton,$vars,$ops){
    // Input check
    if(!is_array($skeleton) || !is_array($vars) || !is_array($ops)){
        echo "Error in randomFillSkeleton: Each input must be an array";
    }
    ...
    if(array_count_values($skeleton)["*"]==1){
        foreach($skeleton as $k=>$v){
            if($v=="*"){                                    // Replace * with variable
                $skeleton[$k] =array_rand(array_flip($vars));               
            }
        }
        return $skeleton;
    }
    ...
    if(array_count_values($skeleton)["*"]==2){
        $var = array_rand(array_flip($vars),2);             // Choose two different variables
        shuffle($var);
        foreach($skeleton as $k=>$v){
            if($v=="*"){                                    // Replace * with variable
    ...
    $operands = getOperands($skeleton);
    // If operands is size 1 then last operation was unary
    if(count($operands)==1){
        return array_merge(randomFillSkeleton($operands[0],$vars,$ops),array("-"));
    }
    // If operands is size 2, then last operation was binary
    $L = $operands[0];
    $L = randomFillSkeleton($L,$vars,$ops);
    $evalL = postfixEvaluateAll($L,$vars);
    ...
    for($i = 0; $i<10000; $i++){
        if($evalL==0 || $evalL == pow(2,$vars)-1){              // If the left operand is a tautology or contradiction, redo constructing it
            $L = randomFillSkeleton($operands[0],$vars,$ops);
			$evalL = postfixEvaluateAll($L,$vars);
        }
        else{
            break;
	...
	$R = $operands[1];
    $R = randomFillSkeleton($R,$vars,$ops);
    $evalR = postfixEvaluateAll($R,$vars);
    ...
    for($i = 0; $i<10000; $i++){
        if($evalR==0 || $evalR==pow(2,$vars)-1){                        // right operand is a tautology or contradiction, redo.
            $R = randomFillSkeleton($operands[1],$vars,$ops);
			$evalR = postfixEvaluateAll($R,$vars);
        }
    ...
    if(count($L)>1 && count($R)>1 && end($L)==end($R) && BINARIES[end($L)]!="ifthen"){
        $allowedops = array_diff($ops,array(end($L)));
        $O = $allowedops[array_rand($allowedops)];
    }
    elseif(count($L)>1 && count($R)<3 && BINARIES[end($L)]!="ifthen"){
        $allowedops = array_diff($ops,array(end($L)));
        $O = $allowedops[array_rand($allowedops)];
    }
    elseif(count($R)>1 && count($L)<3 && BINARIES[end($R)]!="ifthen"){
