...
array_push($allowedmacros,"nCr","nPr","mean","stdev","variance","absmeandev","percentile",
 ...
 "csvdownloadlink","modes","forceonemode","dotplot","anova1way_f","anova1way","anova2way",
 "anova_table","anova2way_f");
...
function anova1way_f(... $arr){
	...
	for($i=0;$i<$numargs;$i++){
		$mean[$i]=mean($args[$i]);
		$ss[$i]=variance($args[$i])*(count($args[$i])-1);
		//$n[$i]=count($args[$i]);
	}
	$gmean=array_sum($mean)/count($mean); //grand mean
	$ssA=variance($mean)*$n[0]*(count($args)-1); //Sum of the square for Factor A
	$ssE=array_sum($ss); //Sum of the square for Residual (Error)
	$ssT=$ssA+$ssE;
	$dfA=count($args)-1;
	$dfE=($n[0]-1)*count($args);
	$dfT=$dfA+$dfE;
...
// Function: anova1way_f(arr, [replication=False])
...
function anova_table(array $array, int $factor = 1, $rep=False, int $roundto=12, string $f1="Factor A", string $f2="Factor B"){
	...
	if ($factor==1){
		$r0=calconarray($array[0],"round(x,$roundto)");
		$r1=calconarray($array[1],"round(x,$roundto)");
		$r2=calconarray($array[2],"round(x,$roundto)");
		...
		elseif($factor==2 && $rep==False){
			...
			$r1=calconarray($array[1],"round(x,$roundto)");
			$r2=calconarray($array[2],"round(x,$roundto)");
			$r3=calconarray($array[3],"round(x,$roundto)");
		...
		elseif($factor==2 && $rep==True){
			...
			$r2=calconarray($array[2],"round(x,$roundto)");
			$r3=calconarray($array[3],"round(x,$roundto)");
			$r4=calconarray($array[4],"round(x,$roundto)");
