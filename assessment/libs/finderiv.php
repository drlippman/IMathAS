...
function finderiv_payout($asset, $types, $possizes, $strikes){
	...
	for ($j=0;$j<$numLevels;$j++) {
		...
		for ($i=0;$i<$numTypes;$i++) {
			...
			if (preg_match("/call/i",$types[$i])==1 ||
				preg_match("/put/i",$types[$i])==1 ||
				preg_match("/forward/i",$types[$i])==1 ||
				preg_match("/bond/i",$types[$i])==1 ) {
				if (!is_numeric($possizes[$i])) {
					echo "finderiv_payout: position size is invalid. Cannot be " . $possizes[$i] . ".\r\n";
					return false;
				}
				if (preg_match("/bond/i",$types[$i])!==1) {
				...
				} elseif (preg_match("/put/i",$types[$i])==1) {
					$total[$j] += $possizes[$i]* max($strikes[$i]-$price,0);
				} elseif (preg_match("/bond/i",$types[$i])==1) {
					$total[$j] += $possizes[$i];
				} elseif (preg_match("/forward/i",$types[$i])==1) {
