...
class htmLawed extends Component
{

    public static function convertdatauris($in) {
        global $CFG,$userid;
        ...
        if (!isset($CFG['GEN']['noFileBrowser'])) {
            ...
            foreach ($matches[3] as $k=>$code) {
                ...
                $key = "ufiles/$userid/pastedimage".\app\components\AppUtility::tzdate("ymd-His",time()).'-'.$k.$okext[$ext];
                filehandler::storecontenttofile($img,$key,"public");
                $in = str_replace($matches[1][$k],filehandler::getuserfileurl($key),$in);
            }
            return $in;
        } else {
            $in = preg_replace('/<img[^>]*src="data:image[^>]*>/','',$in);
    ...
    public static function htmLawed($in, $cf = 1, $spec = array())
    {
    $in = htmLawed::convertdatauris($in);
    ...
    $in = ($cf['balance'] ? htmLawed::hl_bal($in, $cf['keep_bad'], $cf['parent']) : $in);
    ...
    if (!isset($GLOBALS['teacherid'])) {
      $in = htmLawed::remove_instr_only($in);
    }
    return $in;
    // eof
    }

    public static function hl_attrval($v, $p){
    ...
    public static function hl_bal($tx, $do = 1, $in = 'div'){
    ...
    public static function hl_cmtcd($in){
    ...
    public static function hl_ent($in){
    ...
    public static function hl_tag($in){
    ...
    public static function kses($in, $ok_htm, $ok_prots = array('http', 'https', 'ftp', 'news', 'nntp', 'telnet', 'gopher', 'mailto')){
    // kses compat
    ...
    return htmLawed::htmLawed($in, $cf, $ok_htm);
    // eof
    }

    public static function kses_hook($string, &$cf, &$spec){
    // kses compat
    return $string;
    // eof
    }

    public static function remove_instr_only($str) {
