...
function makeanswerbox($anstype, $qn, $la, $options,$multi,$colorbox='') {
	...
	if ($anstype == "number") {
        ...
        if (isset($options['questions'][$qn]) && is_array($options['questions'][$qn])) {$questions = $options['questions'][$qn];} else {$questions = $options['questions'];}
		if (isset($options['questions'][$qn]) && is_array($options['answers'][$qn])) {$answers = $options['answers'][$qn];} else {$answers = $options['answers'];}
		...
		if (isset($options['ansprompt'])) {if (is_array($options['ansprompt'])) {$ansprompt = $options['ansprompt'][$qn];} else {$ansprompt = $options['ansprompt'];}}
		if (isset($options['answerboxsize'])) {if (is_array($options['answerboxsize'])) {$sz = $options['answerboxsize'][$qn];} else {$sz = $options['answerboxsize'];}}
		if (isset($options['hidepreview'])) {if (is_array($options['hidepreview'])){$hidepreview = isset($options['hidepreview'][$qn]) ? $options['hidepreview'][$qn] : '';} else {$hidepreview = $options['hidepreview'];}}
		if (isset($options['answerformat'])) {if (is_array($options['answerformat'])) {$answerformat = $options['answerformat'][$qn];} else {$answerformat = $options['answerformat'];}}
		if (isset($options['answer'])) {if (is_array($options['answer'])) {$answer = $options['answer'][$qn];} else {$answer = $options['answer'];}}
		if (isset($options['reqdecimals'])) {if (is_array($options['reqdecimals'])) {$reqdecimals = $options['reqdecimals'][$qn];} else {$reqdecimals = $options['reqdecimals'];}}
		if (isset($options['displayformat'])) {if (is_array($options['displayformat'])) {$displayformat = $options['displayformat'][$qn];} else {$displayformat = $options['displayformat'];}}
		...
		if (!isset($hidepreview)) {
			$preview .= "<input type=button class='btn btn-primary assessment-btn' value=\"" . _('Preview') . "\" onclick=\"calculate('tc$qn','p$qn','$answerformat')\" /> &nbsp;\n";
		}
		...
		if (isset($answersize)) {
			...
			for ($row=0; $row<$answersize[0]; $row++) {
				$out .= "<tr>";
				for ($col=0; $col<$answersize[1]; $col++) {
                    $lasValue = isset($las[$count]) ? $las[$count] : "";
					$out .= "<td><input class=\"text\" type=\"text\"  size=3 name=\"qn$qn-$count\" id=\"qn$qn-$count\" value=\"{$lasValue}\" autocomplete=\"off\" /></td>\n";
					$count++;
				}
				$out .= "</tr>";
			}
		...
		if (!isset($hidepreview)) {$preview .= "<input type=button class='btn btn-primary assessment-btn' value=\"" . _('Preview') . "\" onclick=\"AMpreview('tc$qn','p$qn')\" /> &nbsp;\n";}
		$preview .= "<span id=p$qn></span>\n";
		...
		if (!is_array($backg) && substr($backg,0,5)=="draw:") {
			...
			$plot = showplot(array($backg,$settings[0],$settings[1],$settings[2],$settings[3],$sclinglbl,$sclinggrid,$settings[6],$settings[7]));
		}
		...
		if ($answerformat[0]=='inequality') {
            		if (in_array('both',$answerformat)) {
               			$out .= "<img src='".AppUtility::getHomeURL()."img/tpineq.gif' onclick='settool(this,$qn,10)' class='sel'/>";
            			$out .= "<img src='".AppUtility::getHomeURL()."img/tpineqdash.gif' onclick='settool(this,$qn,10.2)'/>";
            			$out .= "<img src='".AppUtility::getHomeURL()."img/tpineqparab.gif' onclick='settool(this,$qn,10.3)'/>";
                		$out .= "<img src='".AppUtility::getHomeURL()."img/tpineqparabdash.gif' onclick='settool(this,$qn,10.4)'/>";
                		$def = 10;
            		}
			else if (in_array('parab',$answerformat)) {
               			$out .= "<img src='".AppUtility::getHomeURL()."img/tpineqparab.gif' onclick='settool(this,$qn,10.3)' class='sel'/>";
                		$out .= "<img src='".AppUtility::getHomeURL()."img/tpineqparabdash.gif' onclick='settool(this,$qn,10.4)'/>";
                		$def = 10.3;
            		}  
			else {
			...
			if (count($answerformat)==1 || in_array('line',$answerformat)) {
				$out .= "<img src='".AppUtility::getHomeURL()."img/tpline.gif' onclick='settool(this,$qn,5)' ";
			...
			if ($settings[6]*($settings[3]-$settings[2]) == $settings[7]*($settings[1]-$settings[0])) {
				...
				if (count($answerformat)==1 || in_array('circle',$answerformat)) {
					$out .= "<img src='".AppUtility::getHomeURL()."img/tpcirc.png' onclick='settool(this,$qn,7)' ";
					if (count($answerformat)>1 && $answerformat[1]=='circle') { $out .= 'class="sel" '; $def = 7;}
					$out .= '/>';
				}
			}
			if (count($answerformat)==1 || in_array('dot',$answerformat)) {
				$out .= "<img src='".AppUtility::getHomeURL()."img/tpdot.gif' onclick='settool(this,$qn,1)' ";
				if (count($answerformat)>1 && $answerformat[1]=='dot') { $out .= 'class="sel" '; $def = 1;}
				$out .= '/>';
			}
			if (in_array('opendot',$answerformat)) {
				$out .= "<img src='".AppUtility::getHomeURL()."img/tpodot.gif. onclick='settool(this,$qn,2)' ";
				if (count($answerformat)>1 && $answerformat[1]=='opendot') { $out .= 'class="sel" '; $def = 2;}
				$out .= '/>';
			}
			if (in_array('trig',$answerformat)) {
				$out .= "<img src='".AppUtility::getHomeURL()."img/tpcos.png' onclick='settool(this,$qn,9)' ";
				if (count($answerformat)>1 && $answerformat[1]=='trig') { $out .= 'class="sel" '; $def = 9;}
				$out .= '/>';
				$out .= "<img src='".AppUtility::getHomeURL()."img/tpsin.png' onclick='settool(this,$qn,9.1)'/>";
			}
			if (in_array('vector',$answerformat)) {
				$out .= "<img src='".AppUtility::getHomeURL()."img/tpvec.gif' onclick='settool(this,$qn,5.4)' ";
...
function scorepart($anstype,$qn,$givenans,$options,$multi) {
	...
	if ($anstype == "number") {
				...
				foreach ($aarr as $j=>$anans) {
					if ($anans=='') {
						if (isset($GLOBALS['teacherid'])) {
                            $responseString .= '<p>Debug info: empty, missing or invalid $answer </p>';
						}
						return 0;
					}
		...
		if ($answerformat!="equation" && strpos($answer,'=')!==false) {
            $responseString .= 'Your $answer contains an equal sign, but you do not have $answerformat="equation" set. This question probably will not work right.';
		}
		...
		foreach ($ansarr as $answer) {
			...
			if ($cntnan==20 && isset($GLOBALS['teacherid'])) {
                $responseString .= "<p>Debug info: function evaled to Not-a-number at all test points.  Check $domain</p>";
			}
			...
			if (is_uploaded_file($_FILES["qn$qn"]['tmp_name'])) {
				if ($answerformat=='excel') {
					...
					if ($zip->open($_FILES["qn$qn"]['tmp_name'])) {
                        $responseString .= "opened excel";
						$doc = new DOMDocument();
						if ($doc->loadXML($zip->getFromName('xl/worksheets/sheet1.xml'))) {
                            $responseString .= "read into doc";
						}
				...
				foreach ($answer as $cell=>$val) {
					...
					if (is_numeric($val)) {
						if (abs($val-$doccells[$cell])<.01) {
                            ...
                            $responseString .= "<br/>Cell $cell incorrect";
						}
					} else {
...
function getcolormark($c,$wrongformat=false) {
	...
	if ($c=='ansred') {
		return '<img class="scoreboxicon" src="'.AppUtility::getHomeURL().'img/redx.gif" width="6" height="6"/>';
	} else if ($c=='ansgrn') {
		return '<img class="scoreboxicon" src="'.AppUtility::getHomeURL().'img/gchk.gif" width="8" height="6"/>';
	} else if ($c=='ansorg') {
		return '<img class="scoreboxicon" src="'.AppUtility::getHomeURL().'img/orgx.gif" width="6" height="6"/>';
	} else if ($c=='ansyel') {
		return '<img class="scoreboxicon" src="'.AppUtility::getHomeURL().'img/ychk.gif" width="8" height="6"/>';
	} else {
		return '';
	}
}
