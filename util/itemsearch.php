...
if (isset($_POST['search'])) {
	...
	$srch = $_POST['search'];
	$query = "SELECT DISTINCT imas_users.*,imas_courses.id AS cid FROM imas_users JOIN imas_courses ON imas_users.id=imas_courses.ownerid WHERE imas_courses.id IN ";
	$query .= "(SELECT courseid FROM imas_inlinetext WHERE text LIKE '%$srch%') OR imas_courses.id IN ";
	$query .= "(SELECT courseid FROM imas_linkedtext WHERE text LIKE '%$srch%' OR summary LIKE '%$srch%') ORDER BY imas_users.LastName";
	...
	while ($row = mysql_fetch_assoc($result)) {
		...
		if ($thisperson != $lastperson) {
			echo '<br/><input type="checkbox" name="checked[]" value="'.$row['id'].'" checked="checked"> '.$thisperson ;
			$lastperson= $thisperson;
		}
		echo ' <a href="../course/courses.php?cid='.$row['cid'].'" target="_blank">'.$row['cid'].'</a>';
	}
	echo '</p>';
