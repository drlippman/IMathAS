<?php
	...
	*/
	...
	$runtot = 0;
	//echo "<p>Questions anywhere on system created since 6/10/07</p>\n";
	$query = "SELECT COUNT(DISTINCT imas_questionset.id) AS qcnt,imas_users.LastName,imas_users.id FROM imas_questionset,imas_users WHERE imas_users.id=imas_questionset.ownerid AND imas_questionset.description NOT LIKE '%.pg%' GROUP BY imas_questionset.ownerid  ORDER BY qcnt DESC ";
	$result = mysql_query($query) or die("Query failed : $query " . mysql_error());
	while ($row = mysql_fetch_row($result)) {
		//echo "$row[1]: $row[0]<br/>\n";
		$qs[$row[1].'('.$row[2].')'] = $row[0];
		$runtot += $row[0];
	}
	//echo "Total: $runtot";
	...
	while ($row = mysql_fetch_row($result)) {
		//echo "$row[1]: $row[0]<br/>\n";
		$qsp[$row[1].'('.$row[2].')'] = $row[0];
		$runtot += $row[0];
	}
	//echo "Total: $runtot";

	echo "<p>Questions on system created anytime:  public / total</p>";
	foreach ($qs as $u=>$n) {
		echo "$u: {$qsp[$u]}/$n<br/>\n";
	}
