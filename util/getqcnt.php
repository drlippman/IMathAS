<?php
	...
	*/
	...
	$info = array();
	//echo "<p>Questions anywhere on system created since 6/10/07</p>\n";
	$query = "SELECT COUNT(DISTINCT imas_questionset.id) AS qcnt,imas_users.LastName,imas_users.id,imas_users.email,imas_users.FirstName FROM imas_questionset,imas_users WHERE imas_users.id=imas_questionset.ownerid GROUP BY imas_questionset.ownerid  ORDER BY qcnt DESC ";
	$result = mysql_query($query) or die("Query failed : $query " . mysql_error());
	while ($row = mysql_fetch_row($result)) {
		//echo "$row[1]: $row[0]<br/>\n";
		$qs[$row[2]] = $row[0];
