...
(function() {
    ...
    tinymce.PluginManager.add('asciisvg', function(editor, url) {
        ...
        editor.addCommand('mceAsciisvg', function() {
            ...
            var sscr, isnew, elwidth, elheight, alignm, alt;
            ...
            if (el.nodeName === 'IMG' && (editor.dom.getAttrib(el, "sscr") !== '' || 
                ...
                alt = editor.dom.getAttrib(el, "alt") || "";
            } else {
                isnew = true;
            ...
            var dialogUrl = url + '/asciisvgdlg.htm?v=2' + 
            ...
            if (o.get) {
                ...
                if (imgs !== null) {
                    for (var i = 0; i < imgs.length; i++) {
                        ...
                        var alt = '';
                        if (imgs[i].match(/alt=/)) {
                            alt = imgs[i].replace(/.*alt=\"(.*?)\".*/,"$1");
                        } else {
                            alt = '';
                        }
                        var rep = '<embed type="image/svg+xml" style="' + style + '" sscr="' + decodeURIComponent(sscr) + '" alt="' + alt + '" />';
        ...
        editor.on('PastePreProcess', function(e) {
			...
			// change tag here, convert back in beforesetcontent
            e.content = e.content.replace(/<embed/gi, '<asvg');
        });

        // Handle content processing on load/set
        editor.on('BeforeSetContent', function(o) {
            var imgs = o.content.match(/(<embed[^>]*sscr[^>]*>|<asvg[^>]*sscr[^>]*>)/gi);
            if (imgs !== null) {
                for (var i = 0; i < imgs.length; i++) {
                    var sscr;
                    ...
                    var rep = '<img src="' + AScgiloc + '?sscr=' + encodeURIComponent(sscr) + 
                             ...
                             '" alt="' + alt + '" data-asciisvg="1" data-mce-placeholder="1"/>';
                    o.content = o.content.replace(imgs[i], rep);
                }
            }
            o.content = o.content.replace(/<asvg/gi, '<embed');
        });
