  ...
  tinymce.PluginManager.add(pluginName, function (editor, url) {
      ...
      if (uploadHandler && typeof uploadHandler === 'function') {
        ...
        uploadHandler(
          ...
          true
            ...
            if (!dialogApi.getData().description) {
              dialogApi.setData({ description: file.name });
            }
            dialogApi.showTab("general");
        }).catch((err) => {
            editor.windowManager.alert('Upload failed: ' + err.message);
            document.getElementById('attach-upload-note').style.display = 'none';
            document.getElementById('attach-upload').value = '';
        });
    ...
    function openAttachFileDialog() {
      ...
      if (node.nodeName === 'A' && node.classList.contains('attach')) {
        ...
        initialdescription = editor.selection.getContent();
      }
      const dialogConfig = {
        title: 'Attach File',
        size: 'normal',
