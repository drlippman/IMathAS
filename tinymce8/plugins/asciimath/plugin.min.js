    ...
    tinymce.PluginManager.add('asciimath', function(editor, url) {
        ...
        function nodeToAM(outnode) {
            ...
            if (window.AMTcgilocUseSVG) {
                var toconvstr = str.replace(/(&nbsp;|\u00a0|&#160;)/g,"")
                    .replace(/&gt;/g,">")
                    .replace(/&lt;/g,"<");
                var texstr = "\\displaystyle" + AMTparseAMtoTeX(toconvstr);
                $.get({
                    url: AMTcgiloc + '?' + texstr, 
                    dataType: 'text',
                    success: function (svgText) {
                        ...
                        img.style.verticalAlign = va;
                        img.setAttribute("title", str);
                        img.setAttribute("alt", str);
                        span.appendChild(img);
                        outnode.replaceChildren(span);
                        outnode.title = str;
                    }
                });
            } else if (tinymce.Env.ie) {
