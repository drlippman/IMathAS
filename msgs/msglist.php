...
if (isset($_GET['getstulist'])) {
	...
	if ($isteacher || $msgset == 0 || $msgset == 2) {
		...
		while ($row = $stm->fetch(PDO::FETCH_NUM)) {
			$opts[] = sprintf(
				'<option value="%d"><span class="pii-first-name">%s, %s</span></option>',
				$row[0],
				Sanitize::encodeStringForDisplay($row[2]),
				Sanitize::encodeStringForDisplay($row[1])
			);
	...
	if (isset($_POST['subject']) && isset($_POST['to']) && $_POST['to'] != '0') {
		...
		if (in_array($userid, $studentList)) { // sender is student
			if ($msgset == 2 && in_array($msgToPost, $studentList)) { //only can send to student
				...
				$msgset == 1 && // only can send to teacher
		...
		if ($FCMtokenTo != '') {
			...
			sendFCM(
				$FCMtokenTo,
				_("Msg from:") . ' ' . Sanitize::encodeStringForDisplay($userfullname),
				Sanitize::encodeStringForDisplay($subjectPost),
				$url
			);
		}
		if ($type == 'new') {
		...
		if (isset($_GET['to'])) {
			...
			printf(
				'<span class="pii-full-name">%s, %s</span>',
			...
			if ($filtercid > 0) {
				...
				if ($isteacher || $msgset < 2) {
					...
					if (!$isteacher && !$istutor) {
						if ($ishelpmsg) {
							$query .= "AND (imas_tutors.nomsg & 2)=0 ";
						} else {
							$query .= "AND (imas_tutors.nomsg & 1)=0 ";
						}
					}
...
<form id="qform" method=post action="msglist.php?page=<?php echo $page; ?>&cid=<?php echo $cid; ?>">
	<p><label for="filtercid">Filter by course</label>: <select id="filtercid" onchange="chgfilter()">
			...
			<table class=gb id="myTable">
				<thead>
					<tr>
						<th><span class="sr-only"><?php echo _('Select'); ?></span></th>
