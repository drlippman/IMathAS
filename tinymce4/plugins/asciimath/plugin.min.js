...
(function() {
	...
	tinymce.create('tinymce.plugins.AsciimathPlugin', {
		...
		init : function(ed, url) {
			...
			ed.addCommand('mceAsciimathMQ', function() {
				el = ed.selection.getNode();
				var mqAM = '';
				var isnew = true;
				if (el.nodeName == 'IMG' && ed.dom.getAttrib(el,"data-amq")!==null) {
					mqAM = ed.dom.getAttrib(el,"data-amq");
					isnew = false;
				} else {
					mqAM = ed.selection.getContent();
				}
				ed.windowManager.open({
					...
					buttons: [{
							...
							classes: 'primary',
							onclick: function () {
								var newmqAM = top.tinymce.activeEditor.windowManager.getParams().mqAM;
								var latex = AMTparseAMtoTeX(newmqAM);
								if (isnew) {
									var imgtag = '<img src="'+AMTcgiloc+'?'+encodeURIComponent(latex)+'"';
									imgtag += ' data-amq="'+newmqAM+'" data-mce-placeholder="1" />';
									ed.execCommand('mceInsertContent', false, imgtag);
								} else {
									ed.dom.setAttrib(el,"data-amq", newmqAM);
									ed.dom.setAttrib(el,"src", AMTcgiloc+'?'+encodeURIComponent(latex));
								}
			...
			ed.addButton('asciimathmq', {
				tooltip: 'Add New MathQuill',
				cmd : 'mceAsciimathMQ',
				icon: 'mathadd',
				stateSelector: 'img[data-amq]'
				//image : url + '/img/ed_mathformula2.gif'
			});
			...
			ed.on('BeforeSetContent', function (o) {
				...
				var AMQs = o.content.match(/<span[^>]+AMQ[^>]*>(.*?)<\/span>/gi);
				if (AMQs != null) {
					var amq, imgtag;
					for (var i=0;i<AMQs.length;i++) {
						amq = AMQs[i].replace(/<span[^>]+AMQ[^>]*>(.*?)<\/span>/, "$1").replace(/`/g,"");
						AMTlatex = AMTparseAMtoTeX(amq);
						imgtag = '<img src="'+AMTcgiloc+'?'+encodeURIComponent(AMTlatex)+'"';
						imgtag += ' data-amq="'+amq+'" data-mce-placeholder="1" />';
						o.content = o.content.replace(AMQs[i], imgtag);
					}
				}
			});
			...
			ed.on('PostProcess', function(o) {
				if (o.get) {
					var imgs = o.content.match(/<img[^>]*data-amq[^>]*>/gi);
					if (imgs != null) {
						var span, amq;
						for (var i=0; i<imgs.length; i++) {
							amq = imgs[i].replace(/.*data-amq="(.*?)".*/, "$1");
							span = '<span class="AMQ">`'+amq+"`</span>";
							o.content = o.content.replace(imgs[i], span);
						}
					}
				}
			});
			ed.on('BeforeExecCommand', function (e) {
				if (t.lastAMnode != null) {
					if (e.command == 'Superscript' || e.command == 'Subscript' || e.command == 'Bold' || e.command == 'Subscript' || e.command == 'mceToggleFormat') {
						...
						e.preventDefault();
						ed.execCommand('mceAsciimathDlg');
						return;
					}
				}
