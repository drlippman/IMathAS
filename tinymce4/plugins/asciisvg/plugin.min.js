...
(function() {
	...
	tinymce.create('tinymce.plugins.AsciisvgPlugin', {
				...
				var imgs = o.content.match(/<embed[^>]*sscr[^>]*>/gi);
					if (imgs != null) {
						for (var i=0; i<imgs.length; i++) {
							...
							rep = '<img src="'+ed.getParam('AScgiloc')+'?sscr='+encodeURIComponent(sscr)+'" style="'+style+'" data-sscr="'+decodeURIComponent(sscr)+'" data-asciisvg="1" data-mce-placeholder="1"/>';
							o.content = o.content.replace(imgs[i],rep);
						}
					}
			});
		...
		processresize : function(ed,el) {
			...
			if (width>0 && height>0) {
				sscra = decodeURIComponent(el.getAttribute("data-sscr")).split(',');
				sscra[9] = width;
				sscra[10] = height;
				sscr = sscra.join(',');
