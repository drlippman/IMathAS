...
(function (domGlobals) {
    ...
    var fix = function (originalEvent, data) {
      ...
      if (!event.target && originalEvent && originalEvent.touches) {
        var touch = originalEvent.changedTouches[0] || originalEvent.touches[0];
        // for touch, target is the original element, not element under thumb.
        event.target = domGlobals.document.elementFromPoint(touch.clientX, touch.clientY);
      }
      if (!event.target) {
