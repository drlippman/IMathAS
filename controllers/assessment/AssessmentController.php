...
class AssessmentController extends AppController
{
        ...
        if ($courseId) {
            ...
            if (isset($params['clearattempts'])) {
                ...
                if ($startDate != AppConstant::NUMERIC_ZERO) {
                    $sDate = AppUtility::tzdate(AppConstant::MDY, $startDate);
                    $sTime = AppUtility::tzdate(AppConstant::GIA, $startDate);
                } else {
                    $sDate = AppUtility::tzdate(AppConstant::MDY, time());
                    $sTime = $defStartTime;
                }
                if ($endDate != AppConstant::ALWAYS_TIME) {
                    $eDate = AppUtility::tzdate(AppConstant::MDY, $endDate);
                    $eTime = AppUtility::tzdate(AppConstant::GIA, $endDate);
                } else {
                    $eDate = AppUtility::tzdate(AppConstant::MDY, time() + AppConstant::WEEK_TIME);
                    $eTime = $defTime;
                }
                if ($assessmentData['reviewdate'] > AppConstant::NUMERIC_ZERO) {
                    if ($assessmentData['reviewdate'] == AppConstant::ALWAYS_TIME) {
                        $reviewDate = AppUtility::tzdate(AppConstant::MDY, $assessmentData['enddate'] + AppConstant::WEEK_TIME);
                        $reviewTime = $defTime;
                    } else {
                        $reviewDate = AppUtility::tzdate(AppConstant::MDY, $assessmentData['reviewdate']);
                        $reviewTime = AppUtility::tzdate(AppConstant::GIA, $assessmentData['reviewdate']);
                    }
                } else {
                    $reviewDate = AppUtility::tzdate(AppConstant::MDY, $assessmentData['enddate'] + AppConstant::WEEK_TIME);
                    $reviewTime = $defTime;
                ...
                $getFolder = $this->getParamVal('folder');
                $getFrom = $this->getParamVal('from');
                $page_formActionTag = "add-assessment?block=$block&cid=$courseId";
                if (isset($getId)) {
                    $page_formActionTag .= "&id=" . $getId;
                }
    ...
    public function actionChangeAssessment()
    {
        ...
        if ($line['defpenalty']{0} === 'L') {
            ...
            $skipPenalty = AppConstant::NUMERIC_TEN;
        } else if ($line['defpenalty']{AppConstant::NUMERIC_ZERO} === 'S') {
            $skipPenalty = $line['defpenalty']{AppConstant::NUMERIC_ONE};
            $line['defpenalty'] = substr($line['defpenalty'], AppConstant::NUMERIC_TWO);
        } else {
            $skipPenalty = AppConstant::NUMERIC_ZERO;
        }
        ...
        $agbCats = array();
        CopyItemsUtility::getsubinfo($items, AppConstant::ZERO_VALUE, '', AppConstant::ASSESSMENT, '&nbsp;&nbsp;');
    ...
    public function actionShowTest()
    {
        ...
        $getActas = $this->getParamVal('actas');
        $getId = $this->getParamVal('id');
        ...
        $inexception = false;
        $exceptionduedate = AppConstant::NUMERIC_ZERO;
        /*
...
//                        return $this->redirect('show-test?cid='.$courseId.'&id='.$aid);
            ...
            if ($line == null) {
                ...
                if ($adata['isgroup'] > 0 && !$isreview && !isset($teacherid) && !isset($tutorid)) {
                    ...
                    if (count($query) > 0) {
                        ...
                        if ($adata['isgroup'] == 3) {
                            ...
                            $temp .= "<a href=" . AppUtility::getURLFromHome('course', 'course/course?cid=' . $getCid) . ">Back</a>";
                            return $temp;
                        }
                ...
                $assessmentSessionData['assessmentid'] = $getId;
                ...
                $sessiondata['courseid'] = intval($getCid);
        ...
        if ($isreview) {
            ...
            $superDone = $this->getParamVal('superdone');
            if ($timelimitremaining < 1 || isset($superDone)) {
                $superdone = true;
                $getDone = true;
            }
        ...
        if (isset($getReattempt)) {
            ...
            } else if ($getReattempt == "canimprove") {
        ...
        $getRegen = $this->getParamVal('regen');
        if (isset($getRegen) && $allowregen && $qi[$questions[$getRegen]]['allowregen'] == 1) {
        ...
        $getRegen = $this->getParamVal('regenall');
        if (isset($getRegenAll) && $allowregen) {
            srand();
            if ($getRegenAll == "missed") {
                for ($i = 0; $i < count($questions); $i++) {
            ...
            } else if ($getRegenAll == "fromscratch" && $testsettings['testtype'] == "Practice" && !$isreview) {
        ...
        $getJumpToAns = $this->getParamVal('jumptoans');
        if (isset($getJumpToAns) && $showans === 'J') {
            $tojump = $getJumpToAns;
        ...
        if (!isset($_POST['embedpostback'])) {
            ...
            $addGrpMem = $this->getParamVal('addgrpmem');
            if ((!$sessiondata['isteacher'] || isset($sessiondata['actas'])) && ($testsettings['isgroup'] == 1 || $testsettings['isgroup'] == 2) && ($sessiondata['groupid'] == 0 || isset($addGrpMem))) {
            ...
            if (!$isreview && !$superdone) {
                ...
                if ($timebeforedue < 0) {
                    ...
                    $duetimenote .= '. ';
                    if ($exceptionduedate > 0) {
                        $duetimenote .= 'Due' . " " . AppUtility::tzdate(AppConstant::DATE_TIME, $exceptionduedate);
                    } else {
                        $duetimenote .= 'Due' . " " . AppUtility::tzdate(AppConstant::DATE_TIME, $testsettings['enddate']);
                    }
                } else {
            ...
            if ($testsettings['timelimit'] > 0 && !$isreview && !$superdone) {
                ...
                if ($remaining < 0) {
                    ...
                    if (!isset($getAction) && $restrictedtimelimit) {
        ...
        if (isset($getAction)) {
            ...
            if ($getAction == "scoreall") {
                ...
                if (isset($_POST['saveforlater'])) {
                    ...
                    if ($testsettings['timelimit'] > 0) {
                        $temp .= _('The timelimit will continue to count down');
                    }
                ...
                if (isset($getTo)) { //jump to a problem
                    $toshow = $getTo;
                }
                if ($done || isset($getDone)) { //are all done
                    $shown = $this->showscores($questions, $attempts, $testsettings);
                    $this->endtest($testsettings);
                    if ($shown) {
                        $this->leavetestmsg($sessiondata);
            ...
            } else if ($getAction == 'embeddone') {
                $shown = $this->showscores($questions, $attempts, $testsettings);
                $this->endtest($testsettings);
                if ($shown) {
                    $this->leavetestmsg($sessiondata);
                }
            } else if ($getAction == 'scoreembed') {
                $qn = $_POST['toscore'];
                $colors = array();
                $page = $getPage;
                $divopen = false;
            ...
            if ($testsettings['displaymethod'] == "AllAtOnce") {
                ...
                $temp .= '<script type="text/javascript">var assesspostbackurl="' . 'show-test?embedpostback=true&action=scoreembed&page=' . $getPage . '";</script>';
                ...
                if (strpos($intro, '[PAGE') !== false) {
                    ...
                    $dopage = true;
                    $dovidcontrol = false;
                    $this->showembednavbar($intropages, $getPage);
                    $temp .= "<div class=inset>\n";
                    $temp .= "<a name=\"beginquestions\"></a>\n";
                ...
                for ($i = $qmin; $i < $qmax; $i++) {
                    ...
                    if ($dovidcontrol) {
                        $quesout .= ' style="position: absolute; width:100%; visibility:hidden; top:0px;left:-1000px;" ';
                    }
                    $quesout .= '>';
                    ob_start();
                    embedshowicon($i);
                ...
                if ($dopage == true) {
                    $temp .= '<p>';
                    if ($getPage > 0) {
                        $temp .= '<a href="show-test?page=' . ($getPage - 1) . '">Previous Page</a> ';
                    }
                    if ($getPage < (count($intropages) - 1) / 2 - 1) {
                        if ($getPage > 0) {
                            $temp .= '| ';
                        }
                        $temp .= '<a href="show-test?page=' . ($getPage + 1) . '">Next Page</a>';
                    }
                    $temp .= '</p>';
                }
