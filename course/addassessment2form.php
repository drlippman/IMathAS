...
<div id="app" class="skipmathrender" v-cloak>
	...
	<div v-if="showDisplayDialog" class="fullwrap">
		<div class="dialog-overlay">
	      ...
	      <div class="pane-body">
					<p>
					<?php echo _('<strong>One question at a time</strong>: Students will see one question at a time, and can jump between them in any order</p> <p><strong>All questions at once, or in pages</strong>: In this style, students will typically see all the questions on the screen at once. If desired, you can break the questions into pages on the Add/Remove Questions page by clicking the +Text button and selecting the New Page option.') ?>
					</p>
					<p>
					<?php echo _('<strong>Video Cued</strong>: In this style, the questions pop up automatically at specified times while watching a YouTube video. On the Add/Remove Questions page, after adding the questions to the assessment, click Define Video Cues to specify the video and times to display the questions.') ?>
						</p>
					<?php if (isset($CFG['GEN']['livepollserver'])) { ?>
					<p>
					<?php echo _('<strong>LivePoll</strong>: This is a clicker-style display, requiring students to be in the assessment at the same time as the teacher. The teacher opens a question for students to answer, and results can be viewed live as they are submitted.') ?>
					</p>
					<?php } ?>
	      </div>
			</div>
...
/* Preliminary -possibly remove when there is an agreed solution to translation problem. Replace i18n with _ if needed */
let translationData={
  "values": {
		"On each question immediately": "Unmittelbar nach jeder Frage",
		'At the end of the assessment': "Am Ende des Tests",
		'Total score only at the end': "Nur am Ende des Tests, nur Gesamtpunktzahl",
		'No scores at all':	"Keine Punkte anzeigen",
		'Never': "Nie",
		'Show with the score': "Zusammen mit den Punkten anzeigen",
		'After the last try on a question': "Nach dem letzten Versuch einer Frage",
		'After the last try or Jump to Answer button': "Nach dem letzten Versuch oder beim Klick auf die Antwort-Schaltfläche",
		'After %d tries': "Nach %d Versuchen",
		'After 1 try': "Nach 1 Versuch",
		'After the assessment version is submitted': "Nachdem die Test-Version eingereicht wurde",
		'After the due date': "Nach Fälligkeit des Tests",
		'Immediately - they can always view it': "Sofort - sie können das immer sehen",
		'Immediately': "Sofort"
  }
};
i18n.translator.add(translationData);
/* End Preliminary */
var app = new Vue({
	el: '#app',
  data: <?php echo json_encode($vueData); ?>,
	computed: {
		showscoresOptions: function() {
			var during = {
				'value': 'during',
				'text': i18n('On each question immediately')
			};
			var at_end = {
				'value': 'at_end',
				'text': i18n('At the end of the assessment')
			};
			var total = {
				'value': 'total',
				'text': i18n('Total score only at the end')
			};
			var none = {
				'value': 'none',
				'text': i18n('No scores at all')
			};
		...
		showansOptions: function() {
			...
			if (this.showscores == 'during' && this.defattempts == 1) {
				...
				out = [
					{
						...
						'text': i18n('After the last try on a question')
					},
					{
						'value': 'jump_to_answer',
						'text': i18n('After the last try or Jump to Answer button')
					},
					never
				];
				for (var i=1; i<Math.min(9,this.defattempts);i++) {
					out.push({
						'value': 'after_'+i,
						'text': i>1 ? i18n('After %d tries').replace(/%d/, i) :
													i18n('After 1 try')
					});
				}
				...
				out = [
					{
						'value': 'after_take',
						'text': i18n('After the assessment version is submitted')
					},
					never
				];
			}
		...
		viewInGbOptions: function() {
			...
			var out = [
				{
					'value': 'after_due',
					'text': i18n('After the due date')
				},
				{
					'value': 'immediately',
					'text': i18n('Immediately - they can always view it')
				},
				{
					'value': 'never',
					'text': i18n('Never')
