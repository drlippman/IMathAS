...
if (!(isset($teacherid))) { // loaded by a NON-teacher
	...
	if (isset($_GET['addgrp']) && isset($_POST['grpname']) && isset($_GET['grpsetid'])) {
		...
		if (!isset($_POST['stutoadd'])) { //if not adding students also
			header('Location: ' . $GLOBALS['basesiteurl'] . "/course/managestugrps.php?cid=$cid&grpsetid={$_GET['grpsetid']}");
			exit();
		} else {
				...
				foreach ($page_grpmembers[$grpid] as $uid=>$name) {
					...
					if ($hasuserimg[$uid]==1) {
						if(isset($GLOBALS['CFG']['GEN']['AWSforcoursefiles']) && $GLOBALS['CFG']['GEN']['AWSforcoursefiles'] == true) {
							echo "<img src=\"{$urlmode}s3.amazonaws.com/{$GLOBALS['AWSbucket']}/cfiles/userimg_sm{$uid}.jpg\" style=\"display:none;\" alt=\"User picture\" />";
						} else {
					...
					echo " | <a href=\"managestugrps.php?cid=$cid&grpsetid=$grpsetid&remove=$uid&grpid=$grpid\">Remove from group</a></li>";
				}
			}
			echo '</ul>';
		}
