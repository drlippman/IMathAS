	...
	if (isset($_GET['undo'])) {
		...
		if ($stm->rowCount()==0) {
			...
			if ($row['islatepass']==0) {
				...
				if ($row['itemtype']=='F' && $row['startdate']==0) { //only extended reply - treat like reply exception
					$row['itemtype'] = 'R';
				} else if ($row['itemtype']=='F' && $row['enddate']==0) { //only extended reply - treat like reply exception
					$row['itemtype'] = 'P';
				}
				if ($row['itemtype']=='F' || $row['itemtype']=='P') {
					if ($now > $postby && $row['startdate'] < strtotime("+".$latepasshrs." hours",$now)) {
						$postn = 0;  //too late to un-use
					} else if ($now < $postby) {//before postbydate, return all latepasses
						$postn = $row['islatepass'];
					} else {
						$postn = floor(($row['startdate'] - $now)/($latepasshrs*60*60) + .05);  //if ==$row['islatepass'] then returning all
					}
				}
				if ($row['itemtype']=='F' || $row['itemtype']=='R') {
					if ($now > $replyby && $row['enddate'] < strtotime("+".$latepasshrs." hours",$now)) {
						$replyn = 0;  //too late to un-use
					} else if ($now < $replyby) {//before replybydate, return all latepasses
						$replyn = $row['islatepass'];
					} else {
						$replyn = floor(($row['enddate'] - $now)/($latepasshrs*60*60) + .05);  //if ==$row['islatepass'] then returning all
					}
				}
				...
				if ($postn==0 && $replyn==0) {
					...
					if (($row['itemtype']=='F' && $postn==$row['islatepass'] && $replyn==$row['islatepass']) || ($row['itemtype']=='R' && $replyn==$row['islatepass']) || ($row['itemtype']=='P' && $postn==$row['islatepass'])) {
						...
						$stm->execute(array(':userid'=>$userid, ':assessmentid'=>$fid));
						$toreturn = $row['islatepass'];
					} else {
						if ($row['itemtype']=='F') {
							$toreturn = min($postn,$replyn);
							$tothrs = $toreturn*$latepasshrs;
							$newpostend = strtotime("-".$tothrs." hours", $row['startdate']); // - $toreturn*$latepasshrs*60*60;
							$newreplyend = strtotime("-".$tothrs." hours", $row['enddate']); //$row['enddate'] - $toreturn*$latepasshrs*60*60;
						} else if ($row['itemtype']=='R') {
							$toreturn = $replyn;
							$tothrs = $toreturn*$latepasshrs;
							$newreplyend = strtotime("-".$tothrs." hours", $row['enddate']); //$row['enddate'] - $replyn*$latepasshrs*60*60;
						} else if ($row['itemtype']=='P') {
							$toreturn = $postn;
							$tothrs = $toreturn*$latepasshrs;
							$newpostend = strtotime("-".$tothrs." hours", $row['startdate']); //$row['startdate'] - $postn*$latepasshrs*60*60;
						}
