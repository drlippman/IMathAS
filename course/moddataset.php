...
//4 - CC-BY-SA
	...
	$placeinhead .= '
	   ...
	   $(function() {
		...
		//TODO consider moving the following JS code to a separate file
		// Consider loading only when needed
		...
		//Toggle Input help visibility
		function handleSolverInputHelp() {
			//TODO load from help.php
			//$("#solverinputhelp").load("");

			$("#solverinputhelp").toggle();
		}

		//Toggle Sage help visibility
...
#solvertopbar {
	background: #369; /* Blue from standard menu bar */
	width: 100%;
	color: #fff;
	font-size: 10px;
	font-family: arial;
...
#solverclosebutton:hover {
	color: #f00;
}
#operationselect {
	padding: 2px;
}
div#sagecelloutput, div#sagecell {
	width: 90%;
	align: center;
	margin: 0 auto;
	padding-top: 6px;
}
div#sagecelloutput, div.sagecell_input {
	opacity: 0.5;
}
div#sagecelloutput {
	position: relative;
...
#solvergobutton {
	background-color: #606; /* purple from standard menu bar */
    border: none;
    color: white;
    padding: 2px;
    text-align: center;
...
#solvergobutton:hover {
    box-shadow: 2px 6px 6px 0 rgba(0,0,0,0.24),0 7px 20px 0 rgba(0,0,0,0.19);
}
}
</style>';
...
<div id="solverpopup" style="display: none" class="solverpopup">
	...
	<select id="solveroperation" name="solveroperation">
		<option id="solverchoose" value="solve">Choose
		<option id="solversolve" value="solve">Solve
		<option id="solverdiff" value="diff" selected>Differentiate
