	...
	if ($itemorder == '') {
		...
		for ($i = 0; $i < count($items); $i++) {
			...
			for ($j=0;$j<count($subs);$j++) {
				...
				echo "<td class=c><a href=\"addquestionsreview.php?remove=$i-$j&aid=$aid&cid=$cid\" onclick=\"return confirm('Are you sure you want to remove this question?')\">Remove</a></td>\n";
				echo "</tr>\n";
				$ln++;
			}
			$alt = 1-$alt;
