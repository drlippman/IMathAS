...
$stm->execute(array(':broken'=>$_GET['flag'], ':id'=>$_GET['qsetid']));
if ($stm->rowCount()>0) {
	$ischanged = true;
	if ($_GET['flag']==1) {
		$now = time();
		...
		if (isset($CFG['GEN']['sendquestionproblemsthroughcourse'])) {
			...
			$stm->execute(array(':courseid'=>$CFG['GEN']['sendquestionproblemsthroughcourse'], ':title'=>'Question #'.$_GET['qsetid'].' marked as broken',
				':message'=>"<p>This is an automated message.  $userfullname has marked question #".$_GET['qsetid']." as broken. Hopefully they follow up with you about what they think is wrong with it.</p>",
