...
//	$query .= "WHERE iu.id = istu.userid AND istu.courseid='$cid' AND iu.id=ias.userid AND ias.assessmentid='$aid'";
	...
	$query = "SELECT ig.name,ias.id,ias.userid,ias.bestscores,ias.starttime,ias.endtime,ias.feedback FROM imas_stugroups AS ig ";
	$query .= "JOIN imas_assessment_sessions AS ias ON ig.id=ias.agroupid AND ias.assessmentid='$aid' GROUP BY ias.agroupid";
	$result = mysql_query($query) or die("Query failed : $query" . mysql_error());
