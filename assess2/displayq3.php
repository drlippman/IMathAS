...
MQexperimental??
Autosuggests?
	It'd be nice to not have to dump the same autosuggests several times.

For standard input fields, auto-add scoremarkers based on colorbox classes

Need to:
 - rewrite tips "more" to pull from aria-describedby
 - add listeners to pbtn$qn, rendered into p$qn
 - teach math parser to ignore DNE, oo
 - add catch (Exception) to evalMathParser in master
 - init AutoSuggest
 ...
 x in draw, build drawla[qn] from data-la by converting:
		 ...
		 $la = '[['.implode('],[',$la).']]';
	- revisit make/scorenosolninf to remove tc$qn stuff
...
function displayq($qnidx,$qidx,$seed,$doshowans,$showhints,$attemptn,$partattemptn,$returnqtxt=false,$clearla=false,$qcolors=array(),$stuanswers=array(),$stuanswersval=array()) {
	...
	if ($qdata['qtype']=="multipart" || $qdata['qtype']=='conditional') {
		...
		foreach ($anstypes as $kidx=>$anstype) {
			...
			list($answerbox[$kidx],$entryTips[$kidx],$shanspt[$kidx],$previewloc[$kidx],$jsparam) = makeanswerbox($anstype,$kidx,$laparts[$kidx],$optionsPack,$qnidx+1,$qcol);
			$jsParamsOut[($qnidx+1)*1000+$kidx] = $jsparam;
		}
	} else {
		...
		list($answerbox,$entryTips[0],$shanspt[0],$previewloc,$jsParamsOut[$qnidx]) = makeanswerbox(Sanitize::simpleString($qdata['qtype']),$qnidx,$la,$optionsPack,0,$qcol);
	}
	...
	foreach($entryTips as $iidx=>$entryTip) {
		if ($doshowans && strpos($toevalqtxt,'$showanswerloc')===false && is_array($showanswerloc) && isset($showanswerloc[$iidx])) {
			...
			$sadiv .= '<div>'.$showanswerloc[$iidx].'</div>';
		}
	}
	...
	if ($doshowans && ($qdata['solutionopts']&4)==4 && $qdata['solution']!='') {
		if ($nosabutton) {
			...
			$sadiv .= "<div><input class=\"dsbtn\" type=button value=\""._('Show Detailed Solution')."\" onClick='javascript:$(\"#soln$qnidx\").removeClass(\"hidden\"); rendermathnode(document.getElementById(\"soln$qnidx\"));' />";
			$sadiv .= filter(" <div id=\"soln$qnidx\" class=\"hidden review\" style=\"margin-top:5px;margin-bottom:5px;\">$evaledsoln </div></div>\n");
		}
	}
	$sadiv .= '</div>';
	...
	if ($returnqtxt) {
		return array($returntxt, $jsParamsOut);
	} else {
		// a possible/temporary solution
			echo '<script>
			if (!window.hasOwnProperty("imathasQparams")) {
				window.imathasQparams = {};
			}
			window.imathasQparams = Object.assign(window.imathasQparams, '.
			json_encode($jsParamsOut) .
			');
			</script>';
	}
...
function makeanswerbox($anstype, $qn, $la, $options,$multi,$colorbox='') {
	...
	if ($anstype == "number") {
		...
		if ($displayformat=='hidden') {
			// What's this for? Maybe virtual manipulatives?
	...
	return array($out,$tip,$sa,$preview,$params);
}
